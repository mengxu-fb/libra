    ...
    pub fn unify<'a>(
        ...
        if let Type::Reference(is_mut, bt1) = t1 {
            ...
            return Ok(Type::Reference(
                *is_mut,
                Box::new(self.unify(display_context, variance, bt1.as_ref(), t2)?),
            ));
        }
        if let Type::Reference(is_mut, bt2) = t2 {
            return Ok(Type::Reference(
                *is_mut,
                Box::new(self.unify(display_context, variance, t1, bt2.as_ref())?),
            ));
        }
        ...
        match (t1, t2) {
            (Type::Primitive(p1), Type::Primitive(p2)) => {
                ...
                if matches!(variance, Variance::Allow) && t1.is_number() && t2.is_number() {
            ...
            (Type::Fun(ts1, r1), Type::Fun(ts2, r2)) => {
                return Ok(Type::Fun(
                    self.unify_vec(display_context, variance, ts1, ts2, "functions")?,
                    Box::new(self.unify(display_context, variance, &*r1, &*r2)?),
                ));
            }
            (Type::Struct(m1, s1, ts1), Type::Struct(m2, s2, ts2)) => {
                if m1 == m2 && s1 == s2 {
                    return Ok(Type::Struct(
                        *m1,
                        *s1,
                        self.unify_vec(display_context, variance, ts1, ts2, "structs")?,
                    ));
                }
            }
